...
Once you have defined your metamodel, the next step is to create the actual model instances. This guide covers how to build and structure your model in NN Modeler.
...
NN Modeler automatically infers the **Type** (Class) of each node based on its position in the hierarchy.
...
- If you use the same name `[[Node A]]` in two places, NN Modeler treats them as the same entity appearing in multiple locations (transclusion). If this is not intended, give them unique names like `[[Node A (Context 1)]]`.
