...
This document outlines the core data structures used in NN Modeler, defined in `src/core/types.ts`.
...
export interface RelationshipDefinition {
    widget: WidgetType;
    config?: BinaryConfig | ScaleConfig | SetConfig | CycleConfig | NumberConfig | TextConfig | MermaidConfig;
}

export type WidgetType = 'binary' | 'scale' | 'set' | 'cycle' | 'number' | 'text' | 'mermaid';
```
