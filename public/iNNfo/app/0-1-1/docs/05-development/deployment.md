...
kNN5 is distributed as a **single-page application (SPA)** that can run entirely in the browser without requiring a server. This approach provides maximum portability and privacy.

### How It Works

1. **Build Process**: The application is compiled into a single `index.html` file (~6.6 MB) using Vite with the `vite-plugin-singlefile` plugin
2. **All-in-One**: JavaScript, CSS, and assets are inlined into the HTML file
3. **Local-First**: Works via `file://` protocol - no server required
4. **GitHub Pages Deployment**: The standalone build is deployed to [iNNv0.com](https://innv0.com) via GitHub Pages
...
This creates the standalone application in `app/dist-standalone/`:
- `index.html` - Complete application (~6.6 MB, gzipped: ~1.8 MB)
- `logo.png` - Application logo
- `template.nn.md` - Default template for new models
- `docs/` - Documentation files
- `skills/` - AI assistant skills

### Deploying to iNNv0.com

The official deployment process:
...
2. **Copy to iNNv0.com repository:**
   ```bash
   # Copy the entire dist-standalone directory to the iNNv0.com repo
...
3. **Commit and push:**
   ...
   git add .
   git commit -m "Update kNN5 application"
   git push origin main
   ```

4. **GitHub Pages automatically deploys** the changes, making the application available at [https://innv0.com](https://innv0.com)
