...
This glossary defines the core terminology used throughout kNNowledge. For detailed technical specifications, see [Data Model](./data-model.md) and [File Format](../02-user-guide/file-format.md).
...
**Types:** `binary`, `scale`, `set`, `cycle`, `number`, `text`, `mermaid`, `diagram`, `date`, `color`, `rating`, `url`, `tags`, `toggle-group`, `progress`, `multi-select`, `rich-text`, `code`  
...
**Interfaces:** `BinaryConfig`, `ScaleConfig`, `SetConfig`, `CycleConfig`, `NumberConfig`, `TextConfig`, `MermaidConfig`, `DiagramConfig`, `DateConfig`, `RatingConfig`, `TagsConfig`, `ToggleGroupConfig`, `ProgressConfig`, `JsonConfig`, `MultiSelectConfig`
...
### .nn.md File
The file extension for kNNowledge files. A Markdown file with embedded Mermaid diagrams containing metamodel, model, and relationship definitions.
